...
import {
  ...
  getCourseSubmissionHistorySchema,
 ...
 *     description: Retrieve all submissions for a specific assignment with pagination and filtering
 *     tags: [Submissions]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
/**
 * @swagger
 * /api/submissions/course/{courseId}/history:
 *   get:
...
/**
 * @swagger
 * /api/submissions/{assignmentId}/history:
 *   get:
...
const submissionRouter = Router()
  ...
  .get(
    ...
    catchAsync(authorize(['read_record'])),
    zodValidator(getSubmissionsByAssignmentSchema),
    catchAsync(getSubmissionsByAssignment as RouteType),
  )
  .put(
    '/:assignmentId',
    catchAsync(authorize(['read_record'])),
